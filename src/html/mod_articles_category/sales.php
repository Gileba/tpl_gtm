...
<ul class="category-module<?php echo $moduleclass_sfx; ?> mod-list sales">
	...
	<?php else: ?>
		...
		<?php foreach ($jcFields as $jcfield) { ?>
			<?php $jcFields[$jcfield->name] = $jcfield; ?>
			<?php } ?>
	<?php key_exists("icon", $jcFields) ? ($icon = $jcFields["icon"]->value) : ($icon = null); ?>

			<li class="<?php echo $item->active; ?>">
<?php $images = json_decode($item->images); ?>
				<?php if ($params->get("link_titles") == 1): ?>
					<a class="mod-articles-category-title <?php echo $item->active; ?>" href="<?php echo $item->link; ?>"><?php if (
	!empty($icon)
) {
	print_r($icon);
} ?>

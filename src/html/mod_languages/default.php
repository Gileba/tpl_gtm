...
<?php if ($params->get("dropdown", 0) && !$params->get("dropdownimage", 1)): ?>
	<form name="lang" method="post" action="<?php echo htmlspecialchars_decode(
 	htmlspecialchars(JUri::current(), ENT_COMPAT, "UTF-8"),
 	ENT_NOQUOTES
 ); ?>">
	<select class="inputbox advancedSelect" onchange="document.location.replace(this.value);" >
	<?php foreach ($list as $language): ?>
		<option dir=<?php echo $language->rtl ? '"rtl"' : '"ltr"'; ?> value="<?php echo htmlspecialchars_decode(
...
<?php elseif ($params->get("dropdown", 0) && $params->get("dropdownimage", 1)): ?>
	<div class="btn-group">
		<?php foreach ($list as $language): ?>
			...
			<?php endif; ?>
		<?php endforeach; ?>
		<ul class="<?php echo $params->get("lineheight", 0)
  	? "lang-block"
  	: "lang-inline"; ?> dropdown-menu" dir="<?php echo JFactory::getLanguage()->isRtl() ? "rtl" : "ltr"; ?>">
		<?php foreach ($list as $language): ?>
			<?php if (!$language->active): ?>
				<li>
...
<?php else: ?>
	<ul class="<?php echo $params->get("inline", 1)
 	? "lang-inline"
 	: "lang-block"; ?>" dir="<?php echo JFactory::getLanguage()->isRtl() ? "rtl" : "ltr"; ?>">
	<?php foreach ($list as $language): ?>
		<?php if (!$language->active): ?>
			<li>
			<a href="<?php echo htmlspecialchars_decode(htmlspecialchars($language->link, ENT_QUOTES, "UTF-8"), ENT_NOQUOTES); ?>">
